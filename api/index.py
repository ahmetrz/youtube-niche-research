...
def kaggle_download(dataset, filename):
    ...
    if not env.get("KAGGLE_API_TOKEN"):
        raise HTTPException(500, "KAGGLE_API_TOKEN is not configured")
    subprocess.run(
        ["python", "-m", "kaggle", "datasets", "download", dataset, "-f", filename, "-p", str(DATA)],
        check=True,
        env=env,
    )
    z = DATA / (filename + ".zip")
...
def outliers(limit: int = 50):
    ...
    m = m[(m.videos_last_30_days > 0) & (m.views_last_30_days > 0)].copy()
